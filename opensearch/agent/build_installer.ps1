...
if (-not $SkipBuildExe) {
    $args = @("-m","PyInstaller","--noconfirm","--name","vant-opensearch-agent","--paths","opensearch/agent","opensearch/agent/agent.py")
    if ($BuildMode -eq "onefile") {
        $args = @("-m","PyInstaller","--noconfirm","--onefile","--name","vant-opensearch-agent","--paths","opensearch/agent","opensearch/agent/agent.py")
    }
    & $PythonExe @args
}
...
Copy-Item "opensearch\agent\installer\Install-OpenSearchAgent.ps1" (Join-Path $packageDir "Install-OpenSearchAgent.ps1") -Force
Copy-Item "opensearch\agent\installer\Uninstall-OpenSearchAgent.ps1" (Join-Path $packageDir "Uninstall-OpenSearchAgent.ps1") -Force
Copy-Item "opensearch\agent\installer\config.yaml" (Join-Path $packageDir "config.yaml") -Force
Copy-Item "opensearch\agent\installer\config.windows-server-ad.yaml" (Join-Path $packageDir "config.windows-server-ad.yaml") -Force
Copy-Item "opensearch\agent\installer\config.windows11-ids.yaml" (Join-Path $packageDir "config.windows11-ids.yaml") -Force
