...
class DlpThreatAdmin(admin.ModelAdmin):
    list_display = [
        ...
        "agent_hostname", "classification", "status", "detected_at",
    ]
    list_filter = ["severity", "channel", "classification", "status"]
...
class DlpScanSummaryAdmin(admin.ModelAdmin):
    list_display = ["agent_id", "agent_hostname", "started_at", "files_scanned", "files_flagged", "status"]
    list_filter = ["status"]
